 ...
 */
export const omnaiscopeBackendManager = (() => {
  // singelton for only one possible encapsulated instance of the backend
  ...
  async function getFreePort(): Promise<number> {
    return new Promise((resolve, reject) => {
      ...
      server.listen(0, () => {
        // the OS gets an available port
        const address = server.address();
        if (!isAddressInfo(address)) {
          // check that server.adress is from type AddressInfo
