...
export class StartDataButtonComponent {
  ...
  clearAllData(): void {
    ...
    /** 
     Clear active graph selection by emitting custom event
     GraphComponent listens to this event and calls selectionService.clearSelection()
     */
  ...
  openModal(): void {
    ...
    if (this.advancedMode.enabled()) {
      ...
      const OmnAIScope = this.datasource
        .availableSources()
        .find((s) => s.id === 'omnaiscope');
      if (OmnAIScope) {
